...
import com.egov.applyapp.config.SysConfig;
...
public class LogUtil {
    ...
    public void startWriteLogToSdcard(String filePath) {
        if (SysConfig.isWriteLogToSDcard) {
            if (logWriter == null) {
                try {
     ...
     * @param allParams 需要上传的额外的参数【除了日志以外】
     */
    public void startUploadLog(String strUrl, HashMap<String, String> allParams) {
        if (SysConfig.isUploadLog) {
            if (logUploader == null) {
